...
import ExploitSuccessAnimation from './ExploitSuccessAnimation';
...
export default function AttackModal({ isOpen, onClose, targetName, bounty = 75000 }: { isOpen: boolean, onClose: () => void, targetName: string, bounty?: number }) {
  ...
  const [lastAttackResult, setLastAttackResult] = useState<{pwned: boolean, signature: string} | null>(null);
    ...
    setTimeout(async () => {
      try {
        ...
        if (data.pwned) {
           ...
           setTimeout(() => {
             setLastAttackResult({ pwned: true, signature: data.signature });
  ...
  const handleAnimationComplete = () => {
    ...
    // Optionally close modal or continue
  };
  ...
  return (
    <>
              ...
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                {msg.role === 'system' && (
                  <div className="flex gap-3 max-w-[80%]">
                     ...
                     <div className={`border p-3 whitespace-pre-wrap ${
                       msg.content.includes('SYSTEM COMPROMISED') 
                         ? 'border-green-600 bg-green-950/20 text-green-100 shadow-[0_0_20px_rgba(34,197,94,0.3)]' 
                         : 'border-red-900 bg-red-950/20 text-red-100 shadow-[0_0_10px_rgba(255,0,0,0.1)]'
                     }`}>
                       {msg.content}
                     </div>
                  </div>
      ...
      {lastAttackResult && (
        <ExploitSuccessAnimation 
          isActive={showSuccessAnimation}
          bounty={bounty}
          targetName={targetName}
          signature={lastAttackResult.signature}
